...
# === 新增：计算并打印实际噪声率 ===
clean_label = CIFAR_clean_train.train_label  # 原始干净标签
noisy_label111 = np.array(noisy_label)
clean_label = np.array(clean_label)

assert len(noisy_label111) == len(clean_label), "Label length mismatch!"

actual_noise_rate = np.mean(noisy_label111 != clean_label)
print(f"Requested noise rate: {args.noisy_rate}, Actual noise rate: {actual_noise_rate:.4f} ({actual_noise_rate*100:.2f}%)")
...
# 生成打乱索引
dataset_size = len(clean_label)  # 应等于 len(CIFAR_noisy_train)
perm = torch.randperm(dataset_size).tolist()

# 打乱 clean_label
clean_label_shuffled = clean_label[perm]

# 构造按 perm 顺序排列的子数据集
from torch.utils.data import Subset
shuffled_dataset = Subset(CIFAR_noisy_train, perm)

# DataLoader 不再 shuffle
train_loader = torch.utils.data.DataLoader(
    shuffled_dataset,
    batch_size=args.batch_size,
    shuffle=False,      # ⚠️ 必须为 False
    num_workers=8
)

# train_loader = torch.utils.data.DataLoader(CIFAR_noisy_train, args.batch_size, shuffle=perm, num_workers=8)
test_loader = torch.utils.data.DataLoader(CIFAR_test, args.batch_size, shuffle=False, num_workers=8)
...
def OT_Filter(train_loader, model, optimizer, epoch, clean_label):
    model.eval()
    train_feature = []
    ...
    # 假设 train_label 和 label_propagation 都是长度为 50000 的一维数组
    assert len(clean_label_shuffled) == len(label_propagation), "Label length mismatch!"

    # 转为整数以确保安全比较
    gt_labels = clean_label_shuffled.astype(int)
    pred_labels = label_propagation  # already int

    corrected_acc = (pred_labels == gt_labels).mean()
    print(f"Epoch {epoch}: Label correction accuracy = {corrected_acc * 100:.2f}%")
...
if __name__ == '__main__':
    for epoch in range(args.num_epochs):
        ...
        if epoch<warm_up:
            ...
            OT_Filter(train_loader, net, optimizer, epoch, clean_label)
            test(test_loader, net, epoch)
