...
# 确保保存目录存在
save_dir = "/mnt/zfj/exp_results/OT-filter_tndc"
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, f"{args.dataset}_{args.noisy_type}_{args.noisy_rate}_tndc.json")

acc_dict = {}

if args.dataset=='cifar10':
    root = '/mnt/zfj/dataset/cifar-10-batches-py'
    num_class = 10
    warm_up = 10
    ...
    root = '/mnt/zfj/dataset/cifar-100-python'
    num_class = 100
    warm_up = 30
...
import json
if args.dataset == 'cifar10':
    noisy_label = json.load(open(f'/mnt/zfj/dataset/cifar-10-batches-py/noise_file/cifar10_{args.noisy_type}_{args.noisy_rate}',"r"))
elif args.dataset == 'cifar100':
    noisy_label = json.load(open(f'/mnt/zfj/dataset/cifar-100-python/noise_file/cifar100_{args.noisy_type}_{args.noisy_rate}',"r"))

######  tndc
if args.dataset == 'cifar10':
    tndc_label = json.load(open(f'/mnt/zfj/dataset/dino_mod/cifar10/dino_mod_labels_{args.noisy_type}_{args.noisy_rate}.json',"r"))
elif args.dataset == 'cifar100':
    tndc_label = json.load(open(f'/mnt/zfj/dataset/dino_mod/cifar100/dino_mod_labels_{args.noisy_type}_{args.noisy_rate}.json',"r"))

tndc_label_array = np.array(tndc_label)


CIFAR_noisy_train = cifar_dataset(args.dataset, transform_train, 'train', root, selected = [], noisy_label = noisy_label)
CIFAR_noisy_train_tndc = cifar_dataset(args.dataset, transform_train, 'train', root, selected = [], noisy_label = tndc_label)

# # >>>>>>>>>> 手动 shuffle 所有内容 <<<<<<<<<<
# N = len(CIFAR_noisy_train)  # 50000 for CIFAR
# g = torch.Generator()
# g.manual_seed(args.seed)  # 保证可复现
# perm = torch.randperm(N, generator=g).numpy()

# # Shuffle internal data and labels
# CIFAR_noisy_train.train_data = CIFAR_noisy_train.train_data[perm]
# CIFAR_noisy_train.train_label = [CIFAR_noisy_train.train_label[i] for i in perm]

# CIFAR_noisy_train_tndc.train_data = CIFAR_noisy_train_tndc.train_data[perm]
# CIFAR_noisy_train_tndc.train_label = [CIFAR_noisy_train_tndc.train_label[i] for i in perm]

# # Also shuffle external labels to match
# tndc_label_array = tndc_label_array[perm]

train_loader = torch.utils.data.DataLoader(CIFAR_noisy_train, args.batch_size, shuffle=True, num_workers=2)
train_loader_tndc = torch.utils.data.DataLoader(CIFAR_noisy_train_tndc, args.batch_size, shuffle=True, num_workers=2)
...
def OT_Filter(train_loader, model, optimizer, epoch):
    ...
    ### tndc
    # labeled_sample = train_sample_id[np.where(train_label==tndc_label_array)]
    # unlabeled_sample = train_sample_id[np.where(train_label!=tndc_label_array)]
...
if __name__ == '__main__':
    for epoch in range(args.num_epochs):
        ...
        if epoch < warm_up:
            train_CE(train_loader_tndc, net, optimizer, epoch)
            acc = test(test_loader, net, epoch)
        else:
        ...
        # 记录当前 epoch 的准确率
        acc_dict[str(epoch)] = round(acc, 2)
