...
if args.dataset == 'cifar10':
    ...
    clean_label = json.load(open(f'/mnt/zfj/dataset/cifar-10-batches-py/noise_file/cifar10_sym_0',"r"))
    ...
    clean_label = json.load(open(f'/mnt/zfj/dataset/cifar-100-python/noise_file/cifar100_sym_0',"r"))
...
def OT_Filter(train_loader, model, optimizer, epoch):
    ...
    clean_label_np = np.array(clean_label)
    true_labels_for_samples = clean_label_np[train_sample_id.astype(int)]
    correct = (label_propagation == true_labels_for_samples).sum()
    total = len(label_propagation)
    propagation_acc = correct / total * 100.0
    print(f"Epoch {epoch}: Label Propagation Accuracy = {propagation_acc:.2f}%")
    # <<<<<<<<<< END ADDITION >>>>>>>>>>
