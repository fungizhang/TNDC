    ...
    def __init__(self, dataset, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log=''): 
        ...
        if self.mode=='test':
            ...
            if os.path.exists(noise_file):
                print(f"load noisy labels: {noise_file} !!!")
            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    selected_indices_path = "/home/zhangfeng/zhangfangjiao/projects/CWU/saved/saved_labels_balance/"
                    # 加载保存的数据
                    load_path = os.path.join(selected_indices_path, "selected_data.pth")
                    loaded_data = torch.load(load_path)

                    # 获取数据
                    selected_indices = loaded_data['selected_indices'].detach().tolist()
                    selected_labels = loaded_data['selected_labels'].detach().tolist()

                    self.train_data = train_data[selected_indices]
                    self.noise_label = selected_labels
...
class cifar_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            ...
            all_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform_train, mode="partial",noise_file=self.noise_file)                
            trainloader = DataLoader(
                dataset=all_dataset, 
                batch_size=self.batch_size*2,
                shuffle=True,
