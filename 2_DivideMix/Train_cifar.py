...
if args.label_path is None:
    if args.noise_mode is not None and args.noise_ratio is not None:
        if args.dataset == 'cifar10':
            base_dir = '/mnt/zfj/dataset/cifar-10-batches-py/noise_file'
        elif args.dataset == 'cifar100':
            base_dir = '/mnt/zfj/dataset/cifar-100-python/noise_file'
        else:
            raise ValueError("Unsupported dataset")
        args.label_path = os.path.join(
            base_dir,
            f"{args.dataset}_{args.noise_mode}_{args.noise_ratio}"  # 注意：加 .json
        )
...
for epoch in range(args.num_epochs + 1):
    ...
    result_path = f"/mnt/zfj/exp_results/dividemix/{args.dataset}_{args.noise_mode}_{args.noise_ratio}.json"
