...
class clothing_dataset(Dataset): 
    def __init__(self, root, transform, mode, num_samples=0, pred=[], probability=[], paths=[], num_class=14): 
        ...
        ### zfj
        with open('%s/law_train_labels_conf20.txt'%self.root,'r') as f:
            lines = f.read().splitlines()
            for l in lines:
                entry = l.split()           
                img_path = '%s/'%self.root+entry[0][7:]
                self.law_train_labels[img_path] = int(entry[1])   
    ...
    def __getitem__(self, index):  
        if self.mode=='labeled':
            ...
            # target = self.train_labels[img_path]  
            ### zfj   
            target = self.law_train_labels[img_path]
...
class clothing_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[],paths=[]):        
        if mode=='warmup':
            ...
            warmup_dataset = clothing_dataset(self.root,transform=self.transform_train, mode='all',num_samples=64000)
