...
for i in range(epoch):
    ...
    for j in range(iteration):
        ...
        # 直接使用原始 noisy one-hot 标签（不经过 IPC 优化）
        ...
        # 可选：打印日志
        # print(f'Epoch [{i+1}/{epoch}] Batch [{j+1}/{iteration}] '
        #       f'acc_ori_v1: {acc_ori_v1:.2f}% acc_fcv1: {acc_fcv1:.2f}%')
    ...
    with torch.no_grad():
        ...
        # 注意：由于移除了 IPC，这里不能再用 W1_v1/W1_v2！
        # 所以只评估 EAC 模型和 ensemble
...
filename = f"{dataset}_{noise_class}_{noise_ratio}_noIPC.json"
