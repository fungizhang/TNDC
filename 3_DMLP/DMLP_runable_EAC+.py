...
vval_labels_maxv2 = val_labels_v1
...
# Replace linear models with MLP
...
for i in range(epoch):
    ...
    for j in range(iteration):
        ...
        # ==================== EAC Update (Neural Network Only - MLP) ====================
        ...
        # ==================== Label Swap (Only at last epoch & every epoch_T) ====================
        if i % epoch_T == 0 and i != 0 and i == epoch - 1:
            with torch.no_grad():
                train_labels_v1[start_idx:end_idx] = model_fcv2(batchfeat_v1, apply_softmax=True).detach()
                train_labels_v2[start_idx:end_idx] = model_fcv1(batchfeat_v2, apply_softmax=True).detach()
...
filename = f"{dataset}_{noise_class}_{noise_ratio}_mlp.json"
