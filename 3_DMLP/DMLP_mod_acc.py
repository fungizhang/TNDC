...
for i in range(epoch):
    ...
    for j in range(iteration):
        ...
        # print("======acc_ori_v1:",acc_ori_v1)
        # print("======acc_ori_v2:",acc_ori_v2)
    ...
    # ==================== Test label pre-correction acc====================
    # 将 train_labels_v1 转为硬标签（在训练结束后执行）
    with torch.no_grad():
        # 先 softmax 得到概率分布（因为当前是 logits-like）
        prob_v1 = F.softmax(train_labels_v1, dim=1)  # shape: (train_num, num_classes)
        pred_hard_v1 = torch.argmax(prob_v1, dim=1)  # shape: (train_num,)

    # 获取对应的干净标签（ground truth）
    clean_train_labels = labels_correct[idx_to_train]  # 已在 GPU 上

    # 计算准确率
    correct_count = (pred_hard_v1 == clean_train_labels).sum().item()
    total_count = clean_train_labels.size(0)
    accuracy = correct_count / total_count * 100.0

    print(f"修正后硬标签 vs 真实标签的准确率: {accuracy:.2f}%")
