...
if args.label_path is None:
    if args.noise_mode is not None and args.noise_ratio is not None:
        if args.data_name == 'cifar10':
            base_dir = './datasets/cifar-10-batches-py/noise_file'
        elif args.data_name == 'cifar100':
            base_dir = './datasets/cifar-100-python/noise_file'
        else:
            raise ValueError("Unsupported data_name")
        # 假设文件名为 cifar10_sym_0.2.json 等（注意：保留 .json）
        args.label_path = os.path.join(
            base_dir,
            f"{args.data_name}_{args.noise_mode}_{args.noise_ratio}"
        )
...
def main():
    ...
    # 构造文件名
    if args.noise_mode is not None and args.noise_ratio is not None:
        filename = f"{args.data_name}_{args.noise_mode}_{args.noise_ratio}.json"
    else:
