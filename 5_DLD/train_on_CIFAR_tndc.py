...
def train(diffusion_model, train_dataset, test_dataset, model_path, args, vit_fp, fp_dim):
    ...
    ######  tndc
    if dataset == 'cifar10':
        noisy_targets = json.load(open(f'./datasets/dino_mod/cifar10/dino_mod_labels_{noise_class}_{noise_ratio}.json',"r"))
    elif dataset == 'cifar100':
        noisy_targets = json.load(open(f'./datasets/dino_mod/cifar100/dino_mod_labels_{noise_class}_{noise_ratio}.json',"r"))
    ...
    if dataset == 'cifar10':
        data_dir  = os.path.join(os.getcwd(), './datasets/cifar-10-batches-py')
    else:
        data_dir  = os.path.join(os.getcwd(), './datasets/cifar-100-python')
    ...
    max_accuracy = 0.0
    test_acc_dict = {}  # 用于记录每个 epoch 的 test accuracy
    ...
    for epoch in range(n_epochs):
        ...
        if epoch >= warmup_epochs:
            ...
            # 记录到字典（key 是字符串形式的 epoch）
            test_acc_dict[str(epoch)] = float(test_acc)

            if test_acc > max_accuracy:
                # ... [模型保存逻辑不变] ...
                max_accuracy = test_acc

            # === 保存为扁平 JSON：{"0": 9.9, "1": 31.31, ...} ===
            
            result_dir = "./exp_results/DLD_tndc"
            os.makedirs(result_dir, exist_ok=True)
            json_filename = f"{dataset}_{noise_class}_{noise_ratio}_tndc.json"
...
if __name__ == "__main__":
    ...
    if dataset == 'cifar10':
        n_class = 10
        train_dataset_cifar = torchvision.datasets.CIFAR10(root='./datasets', train=True, download=True)
        test_dataset_cifar = torchvision.datasets.CIFAR10(root='./datasets', train=False, download=True)
        # Data normalization parameters
        CIFAR_MEAN = (0.4914, 0.4822, 0.4465)
        CIFAR_STD = (0.2023, 0.1994, 0.2010)
    elif dataset == 'cifar100':
        n_class = 100
        train_dataset_cifar = torchvision.datasets.CIFAR100(root='./datasets', train=True, download=True)
        test_dataset_cifar = torchvision.datasets.CIFAR100(root='./datasets', train=False, download=True)
        CIFAR_MEAN = (0.4914, 0.4822, 0.4465)
        CIFAR_STD = (0.2023, 0.1994, 0.2010)
    else:
