...
def train(diffusion_model, train_dataset, val_dataset, test_dataset, model_path, args, real_fp, clean_train_targets):
    device = diffusion_model.device
    n_class = diffusion_model.n_class
    n_epochs = args.nepoch
    k = args.k
    ...
    # 转为 tensor 并放到 device（用于索引）
    clean_train_targets = torch.tensor(clean_train_targets, dtype=torch.long).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(enumerate(train_loader), total=len(train_loader), desc=f'train diffusion epoch {epoch}', ncols=120) as pbar:
            for i, data_batch in pbar:
                ...
                # >>> 新增：累加修正标签的正确数 <<<
                clean_labels_batch = clean_train_targets[data_indices]
                total_corrected += (y_labels_batch == clean_labels_batch).sum().item()
                total_samples += y_labels_batch.size(0)
        ...
        # >>> 计算并打印整个 epoch 的修正标签准确率 <<<
        epoch_corr_acc = 100.0 * total_corrected / total_samples
        print(f"Epoch {epoch} - KNN-corrected label accuracy: {epoch_corr_acc:.2f}%")
...
if __name__ == "__main__":
    ...
    # clean_train_targets = train_dataset_cifar.targets[:50000]
    ...
    # noise_label = np.load('/mnt/zfj/dataset/cifar-10-batches-py/noise_file/cifar10_idn_0.4')
    ...
    # dataset_name = dataset  # 'cifar10' or 'cifar100'
    # noise_mode = 'idn'
    # 从 args.noise_type 提取 noise_ratio，例如 'cifar10-idn-0.6' 或类似格式
    # 但原 args.noise_type 是 'cifar10-1-0.35'，不符合；所以根据代码1硬设 ratio=0.6？
    # 但更合理的是：让 noise_ratio 可配置，或从文件名解析

    # 为匹配代码1，假设 noise_ratio 固定为 0.6（或从 args 解析）
    # 假设格式如 cifar10-idn-0.6
    parts = args.noise_type.split('-')
    dataset_name = parts[0]
    noise_mode = parts[1]
    noise_ratio = float(parts[2])
       


    # 使用 dataloader_cifar 加载带噪数据集（eval 模式）
    loader = dataloader.cifar_dataloader(
        ...
        # root_dir='/mnt/zfj/dataset/cifar-10-batches-py',  # 与 torchvision 路径一致
        root_dir='/mnt/zfj/dataset/cifar-100-python',  # 与 torchvision 路径一致
        model='dino'  # 任意，因为只取标签
    )
    train_loader_for_noise = loader.run('eval')  # 获取带噪训练集

    # 提取噪声标签和干净标签
    noise_label = np.array(train_loader_for_noise.dataset.noise_label)
    clean_train_targets = np.array(train_loader_for_noise.dataset.clean_label)

    # 更新 train_dataset 的标签为 noise_label
    train_dataset.update_label(noise_label)
    print(f'Training on noise label: {dataset_name}, mode={noise_mode}, ratio={noise_ratio}')
    ...
    train(diffusion_model, train_dataset, val_dataset, test_dataset, model_path, args, real_fp=real_fp, clean_train_targets=clean_train_targets)
