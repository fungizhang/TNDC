...
def train(diffusion_model, train_dataset, val_dataset, test_dataset, model_path, args, real_fp, clean_targets):
    ...
    if args.is_tndc==0:
        result_file = os.path.join(result_dir, f"{args.noise_type}.json")
    else:
        result_file = os.path.join(result_dir, f"{args.noise_type}_tndc.json")
    # 初始化记录字典（如果文件已存在则加载）
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(enumerate(train_loader), total=len(train_loader), desc=f'train diffusion epoch {epoch}', ncols=120) as pbar:
            for i, data_batch in pbar:
                ...
                # 直接使用原始标签（不进行 KNN 修正）
                y_labels_batch = y_batch.to(device)

                # 每个样本权重平均分配：1 / batch_size
                batch_size_current = y_labels_batch.size(0)
                sample_weight = torch.full((batch_size_current,), 1.0 / batch_size_current, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--noise_type', default='cifar10-sym-0.2', help='noise label file', type=str)
    parser.add_argument('--is_tndc', default=1, help='is_tndc', type=int)
    ...
    parser.add_argument("--device", default='cuda:4', help="which GPU to use", type=str)
